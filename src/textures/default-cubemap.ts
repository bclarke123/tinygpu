...
export class DefaultCubemapTexture extends Texture {
  ...
  public static getInstance(
    device: GPUDevice,
    defaultColor?: Color,
  ): DefaultCubemapTexture {
    if (!DefaultCubemapTexture._instance) {
      DefaultCubemapTexture._instance = new DefaultCubemapTexture(
        device,
        defaultColor,
      );
    }
  ...
  get label(): string {
    return "Default Cubemap Texture";
  }
  ...
  get dimension(): GPUTextureDimension {
    return "2d";
  } // The underlying GPUTexture is a 2D array

  get texture(): GPUTexture {
    if (!this._texture) {
      throw new Error(
        "DefaultCubemapTexture: GPUTexture not available. Call upload() or ensure device was passed to getInstance().",
      );
    }
